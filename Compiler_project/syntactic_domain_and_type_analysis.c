...
bool exprAssign(Ret *r){
	...
	if(exprUnary(&rDst)){
		if(consume(ASSIGN)){
			if(exprAssign(r)){
                if(!rDst.lval) tkerr("Dsetinatia asignarii trebuie sa fie o valoare de stanga.\n");
                if(rDst.ct) tkerr("Destinatia asignarii nu poate fi constanta.\n");
                if(!canBeScalar(&rDst)) tkerr("Destinatia asignarii trebuie sa fie scalara.\n");
                if(!canBeScalar(r)) tkerr("Sursa asignarii trebuie sa fie scalara.\n");
                if(!convTo(&r->type, &rDst.type)) tkerr("Sursa asignarii nu poate fi convertita la destinatie.\n");
                r->lval = false;
                r->ct = true;
				return true;
			}else tkerr("Lipseste expresia de dupa =\n");
...
bool exprPrimary(Ret *r){
    ...
    if(consume(LPAR)){
        if(s->kind != SK_FN) tkerr("Doar o functie poate fi apelata.\n");

        Ret rArg;
        Symbol *param = s->fn.params;

        if(expr(&rArg)){
            if(!param) tkerr("Prea multe argumente in apelul functiei.\n");
...
bool stm(){
	...
	if(consume(IF)){
		if(consume(LPAR)){
			if(expr(&rCond)){
                if(!canBeScalar(&rCond)) tkerr("Conditia din if trebuie sa fie o valoare scalara.\n");
				if(consume(RPAR)){
					if(stm()){
						if(consume(ELSE)){
							if(stm()){
	...
	if(consume(WHILE)){
		if(consume(LPAR)){
			if(expr(&rCond)){
                if(!canBeScalar(&rCond)) tkerr("Conditia din while trebuie sa fie o valoare scalara.\n");
				if(consume(RPAR)){
					if(stm(rCond, rExpr)){
						return true;
					}else tkerr("Lipseste instructiunea de dupa )\n");
	...
	if(consume(RETURN)){
		if(expr(&rExpr)){
            if(owner->type.tb == TB_VOID) tkerr("O functie vida nu poate returna o valoare.\n");
            if(!canBeScalar(&rExpr)) tkerr("Valoarea returnata trebuie sa fie scalara.\n");
            if(!convTo(&rExpr.type, &owner->type)) tkerr("Nu se poate converti tipul returnat de expresie la tipul returnat de functie.\n");
        }else 
            if(owner->type.tb!=TB_VOID) tkerr("O functie nevida trebuie sa returneze o valoare");
		if(consume(SEMICOLON)){
			return true;
